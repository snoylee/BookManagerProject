...
public class PageWidget extends View{
	...
	private int mHeight = 800;//屏幕高度
	private int mCornerX = 0; //拖曳点对应的页脚X轴坐�?
	private int mCornerY = 0;//拖曳点对应的页脚Y轴坐�?
	private Path mPath0;
	private Path mPath1;
	Bitmap mCurPageBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.bg); //当前�?
	...
	private void calcPoints() {
		...
		// 当mBezierStart1.x < 0或�?mBezierStart1.x > 480�?
		// 如果继续翻页，会出现BUG故在此限�?
...
//		mFolderShadowDrawable.draw(canvas);
	...
	private void startAnimation(int delayMillis) {
		int dx, dy;
		// dx 水平方向滑动的距离，负�?会使滚动向左滚动
		// dy 垂直方向滑动的距离，负�?会使滚动向上滚动�?
		if (mCornerX > 0) {
		...
		if (mCornerY > 0) {
			...
			dy = (int) (1 - mTouch.y); //防止mTouch.y�?��变为0
		}
