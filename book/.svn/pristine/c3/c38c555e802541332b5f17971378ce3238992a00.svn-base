...
public class ApiClient {
	 ...
	 * @param handler
	 */
	public static void getIndexInfo(Handler handler) {
		JsonHandler jsonHandler = new JsonHandler(handler);
		RestClient.get(UrlDic.INDEX_URL, jsonHandler);
	}
	...
	public static void getUserInfo(final Handler handler){
		JsonHandler jsonHandler = new JsonHandler(handler);
		RestClient.get(UrlDic.GET_USER_INFO_URL, jsonHandler);
	}
	/**
	 * 
	 * handler 内传传递一个handler 访问网络 时 回调返回的结果
	 *
	 */
